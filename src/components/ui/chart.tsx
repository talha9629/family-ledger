...
import { sanitizeColor, escapeForCSS } from "@/lib/colorValidation";
...
${colorConfig
  .map(([key, itemConfig]) => {
    ...
    // Sanitize color values to prevent CSS/XSS injection
    const color = rawColor ? sanitizeColor(rawColor) : null;
    // Also escape the key to be safe
    const safeKey = escapeForCSS(key);
